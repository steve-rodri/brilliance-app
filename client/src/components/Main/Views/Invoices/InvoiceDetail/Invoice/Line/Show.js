...
export default function Show(props){
  const { line, index, length, styleCell } = props
  const c = contents(line.item)
  return (
    <tr key={line.id} className="Line">
      <td style={styleCell(index, length)} className="Invoice--cell Line--quantity"><p>{line.quantity}</p></td>
      ...
      <td style={styleCell(index, length)} className="Invoice--cell Line--price">{line.price > 0? numeral(line.price).format('$0,0.00') : null }</td>
    </tr>
  )
}
