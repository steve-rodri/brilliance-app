...
export default class Events extends Component {
  ...
  setEvents = async prevProps => {
    if (prevProps) {
      ...
      if (prevLocation && prevLocation.search)
        prevQueryStr = prevLocation.search;
  ...
  addEvent = async (formData, sendUpdates) => {
    ...
    if (calendarId) {
      const newGoogleEvent = await GOOGLE.createEvent(
        calendarId,
  ...
  updateEvent = async (e, data, sendUpdates) => {
    ...
    if (calendarId) {
      ...
      if (!sendUpdates || sendUpdates === "none")
        this.ajaxOptions.sendUpdates = "none";
      ...
      let formatted = await formatFromGoogle(
        googleEvent,
        this.ajaxOptions,
        updatedEvent
      );
    ...
    return (
      <Switch>
        ...
        <Route
          exact
          path={`${match.path}/new`}
          render={props => this.Create(props)}
        />
