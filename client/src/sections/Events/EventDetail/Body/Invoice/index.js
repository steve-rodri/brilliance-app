...
export default class Invoice extends Component {
    ...
    if (inv) {
      return (
        <div
          ...
          style={this.stylePaymentStatus(inv.paymentStatus)}
        >
  ...
  status = inv => {
    return <Fragment>{this.paymentStatus(inv)}</Fragment>;
  };
    ...
    if (evt.invoice) {
      ...
      return (
        <div className="Event-Invoice--no-invoice">
          <h1>--</h1>
          <AddNew linkPath={addNewPath()} type="Invoice" />
        </div>
      );
    }
  };
  ...
  render() {
    ...
    return (
      <div
        style={styleComp("Invoice")}
        className="EventDetail-Body--component EventDetail-Body--invoice"
      >
        {!mobile ? (
