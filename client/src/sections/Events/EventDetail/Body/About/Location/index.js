    ...
    return (
      <div className="CreateLocation">
        <div className="CreateLocation--header">
          ...
          <h2>{`${mode === "create" ? "Create" : "Edit"} ${
            view === "addressForm" ? "Address" : "Location"
          }`}</h2>
        </div>
        <div className="CreateLocation--content">{this.content()}</div>
        <div className="CreateLocation--footer">{this.footer()}</div>
      </div>
    );
  }
}
