...
export default class Schedule extends Component {
  ...
  findAllUserEvents = async () => {
    ...
    if (user) {
      const data = await event.batch(
        { page, email: user.profile.email },
        this.ajaxOptions
      );
  ...
  findUpcomingUserEvents = async () => {
    ...
    const upcomingEvents = userEvents.filter(evt => {
      ...
      return (
        start(evt) &&
        (moment(start(evt)).isSameOrAfter(now) || now.isSameOrBefore(end(evt)))
      );
    });
    if (upcomingEvents.length > 0) {
      upcomingEvents.reverse();
      const uEvents = await Promise.all(
        upcomingEvents.map(async evt => await this.syncWithGoogle(evt))
      );
  ...
  updateEvent = async (e, data) => {
    ...
    if (e.gcId) {
      googleEvent = await GOOGLE.patchEvent(
        calendarId,
        e.gcId,
        formatToGoogle(updatedEvent),
        this.ajaxOptions
      );
    }
    if (!e.gcId) {
      googleEvent = await GOOGLE.createEvent(
        calendarId,
        formatToGoogle(updatedEvent),
        this.ajaxOptions
      );
