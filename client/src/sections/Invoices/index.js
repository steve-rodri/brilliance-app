...
export default class Invoices extends Component {
  ...
  setInvoices = async prevProps => {
    ...
    if (prevProps) {
      ...
      if (prevLocation && prevLocation.search)
        prevQueries = prevLocation.search;
      if (nextLocation && nextLocation.search)
        nextQueries = nextLocation.search;
  ...
  setByDate = async options => {
    ...
    if (options && options.category !== null && category)
      searchLabel = `${category} - ${date}`;
    if (options && options.client !== null && client)
      searchLabel = `${client} - ${date}`;
    if (options && options.query !== null && query)
      searchLabel = `${query} - ${date}`;
