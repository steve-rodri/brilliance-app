...
export default function Show(props) {
  const { line, index, length, mobile, showQty } = props;
  const c = itemContents(line.item);
  return (
    <tr key={line.id} className="Line">
      {showQty ? (
        <td style={(index, length)} className="Invoice--cell Line--quantity">
          <p>{line.quantity}</p>
        </td>
      ) : null}
      ...
      {!mobile ? (
        <td style={(index, length)} className="Invoice--cell Line--price">
          <p>{line.price > 0 ? numeral(line.price).format("$0,0.00") : null}</p>
        </td>
      ) : null}
    </tr>
  );
