...
export default function Header(props) {
  ...
  function displayJob() {
    if (!mobile && !editMode) {
      return <div className="InvoiceDetail-header--view-job">{job()}</div>;
    }
  }
    ...
    if (inv) {
      ...
      return (
        <div className="InvoiceDetail-header--title">
          {c ? (
            <h2
              className="InvoiceDetail-header--client"
              style={
                !mobile && c && e
                  ? { justifySelf: "right" }
                  : { justifySelf: "center" }
          ...
          {e ? (
            <h2
              className="InvoiceDetail-header--date"
              style={
                !mobile && c && e
                  ? { justifySelf: "left" }
                  : { justifySelf: "center" }
            ...
            >
              {moment(e.start).format("MMMM Do YYYY")}
            </h2>
          ) : null}
        </div>
      );
  ...
  function edit() {
    return (
      <div className="InvoiceDetail-header--icon right" onClick={props.edit}>
        {pencilIcon("2x")}
      </div>
    );
  }
  ...
  function submit() {
    return (
      <div className="InvoiceDetail-header--icon right" onClick={props.submit}>
        {checkIcon("2x")}
      </div>
    );
  }
