  ...
  setFields = () => {
    ...
    if (!inv) {
      ...
      fieldNames.forEach(field =>
        inv[field] ? this.setField(field, inv[field]) : null
      );
    }
  };
  ...
  addItem = async (data, category) => {
    ...
    this.setState(
      prevState => ({
        inv: {
          ...prevState.inv,
          lines: [...prevState.inv.lines, newLine]
        },
        fields: {
          ...prevState.fields,
          lines: [...prevState.fields.lines, newLine]
        },
        showItemModal: false
      }),
      async () => await this.updateSummary()
      ...
      switch (name) {
        ...
        case "quantity":
          return {
            ...
            price: parseFloat(
              linePrice({ ...line, quantity: parseInt(value) }, inv.kind, {
                override: true
              })
            )
          };
